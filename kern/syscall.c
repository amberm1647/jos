...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if ((uint32_t) va >= UTOP || (uintptr_t) va & 0xfff)
        return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if ((uint32_t) srcva >= UTOP || (uintptr_t) srcva & 0xfff)
        return -E_INVAL;

    if ((uint32_t) dstva >= UTOP || (uintptr_t) dstva & 0xfff)
        return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if (srcva < (void *) UTOP) {
        ...
        if (page_lookup(curenv->env_pgdir, srcva, 0) == NULL) {
            cprintf("sys_ipc_try_send: out of memory to map srcva\n");
            return -E_INVAL;
        }
    }
...
sys_ipc_recv(void *dstva)
{
    ...
    else curenv->env_ipc_dstva = (void *) UTOP;
