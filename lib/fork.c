...
pgfault(struct UTrapframe *utf)
{
    ...
    if (!(err & FEC_WR) || !(uvpt[PGNUM(addr)] & PTE_COW))
        panic("pgfault: faulting access at %p was not on write or to COW page\n", addr);
    ...
    if ((r = sys_page_map(0, PFTEMP, 0,
            (void *) PTE_ADDR(addr), PTE_U | PTE_W | PTE_P)) < 0)
...
duppage(envid_t envid, unsigned pn)
{
    ...
    if (pte & (PTE_W | PTE_COW)) {
        if ((r = sys_page_map(0, (void *) (pn*PGSIZE), envid,
            ...
            panic("duppage child COW: sys_page_map: %e\n", r);

        if ((r = sys_page_map(0, (void *) (pn*PGSIZE), 0,
                (void *) (pn*PGSIZE), PTE_U | PTE_P | PTE_COW)) < 0)
            panic("duppage parent COW: sys_page_map: %e\n", r);
    }
    else {
        if ((r = sys_page_map(0, (void *) (pn*PGSIZE), envid,
                (void *) (pn*PGSIZE), PTE_U | PTE_P)) < 0)
            panic("duppage child RO: sys_page_map: %e\n", r);
    }
